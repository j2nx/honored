...
from honored.config import Authorization

class Boosts: 
 
  def get_level(boosts: int): 
   async def predicate(ctx: commands.Context): 
    if ctx.guild.premium_subscription_count < boosts: await ctx.warn( f"This server needs to have more than **{boosts}** boosts in order to use this command") 
...
class Joint: 
  ...
  def check_joint():
   async def predicate(ctx: commands.Context): 
    ...
    if not check: await ctx.bot.ext.warn(ctx, f"This server **doesn't** have a **joint**. Use `{ctx.clean_prefix}joint toggle` to get one")    
  ...
  def joint_owner(): 
   async def predicate(ctx: commands.Context): 
    check = await ctx.bot.db.fetchrow("SELECT * FROM joint WHERE guild_id = $1", ctx.guild.id)
    if check["holder"] != ctx.author.id: await ctx.warn( f"You don't have the **joint**. Steal it from <@{check['holder']}>")
...
class Mod: 
  ...
  def is_mod_configured(): 
   async def predicate(ctx: commands.Context): 
    ...
    if not check: 
     await ctx.warn( f"Moderation isn't **enabled** in this server. Enable it using `{ctx.clean_prefix}setmod` command")
     return False
    return True
   return commands.check(predicate)
  ...
  async def check_role_position(ctx: commands.Context, role: discord.Role) -> bool: 
   if (role.position >= ctx.author.top_role.position and ctx.author.id != ctx.guild.owner_id) or not role.is_assignable(): 
    await ctx.warn( "I cannot manage this role for you")
    return False 
   return True
  ...
  async def check_hieracy(ctx: commands.Context, member: discord.Member) -> bool: 
   ...
   if (ctx.author.top_role.position <= member.top_role.position and ctx.guild.owner_id != ctx.author.id) or ctx.guild.me.top_role <= member.top_role or (member.id == ctx.guild.owner_id and ctx.author.id != member.id): 
    await ctx.warn( "You can't do this action on **{}**".format(member))
    return False  
   return True 
  ...
  def server_owner(): 
   async def predicate(ctx: commands.Context): 
    if ctx.author.id != ctx.guild.owner_id: 
      await ctx.warn( f"This command can be used only by **{ctx.guild.owner}**") 
      return False 
    return True   
   return commands.check(predicate)   
