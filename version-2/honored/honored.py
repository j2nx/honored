...
class honored(Worker):
    ...
    async def on_connect(self) -> None:
        ...
        async def create_db(self: commands.AutoShardedBot): 
          ...
          await self.db.execute("CREATE TABLE IF NOT EXISTS pfps (guild_id BIGINT, user_id BIGINT, avatar_url TEXT, PRIMARY KEY (guild_id, user_id);")
          ...
          await self.db.execute("CREATE TABLE IF NOT EXISTS blacklisted_users (user_id BIGINT PRIMARY KEY);")
          await self.db.execute("CREATE TABLE lastfm (user_id BIGINT PRIMARY KEY, username TEXT);")
          await self.bot.db.execute("CREATE TABLE IF NOT EXISTS stream_title (title TEXT);")
          await self.bot.db("CREATE TABLE IF NOT EXISTS active_guilds (guild_id BIGINT PRIMARY KEY, channel_id BIGINT);")
